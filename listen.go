...
type OnionService struct {
	...
	// onionSerivceutil/ed25519.KeyPair when Version3 is true.
	...
	// if onionSerivce had to generate them.
	...
	// The onionSerivce object that created this. Needed for Close.
	onionSerivce *onionSerivce
}
...
type ListenConf struct {
	...
	// onionSerivceutil/ed25519.KeyPair, a
	...
	// generated by onionSerivce for that user. If this is empty there is no
	// authentication.
	ClientAuths map[string]string
	...
	// tells onionSerivce not to return the generated private key. This value is ignored
	// if Key is not nil.
	DiscardKey bool
	...
	// NonAnonymous must be true if onionSerivce options HiddenServiceSingleHopMode and
...
func (t *onionSerivce) Listen(ctx context.Context, conf *ListenConf) (*OnionService, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	// Create the service up here and make sure we close it no matter the error within
	svc := &OnionService{onionSerivce: t, CloseLocalListenerOnClose: conf.LocalListener == nil}
	var err error
...
func (o *OnionService) Close() (err error) {
	o.onionSerivce.Debugf("Closing onion %v", o)
	// Delete the onion first
	if o.ID != "" {
		err = o.onionSerivce.Control.DelOnion(o.ID)
		o.ID = ""
	}
	...
	if err != nil {
		o.onionSerivce.Debugf("Failed closing onion: %v", err)
	}
	return
}
